#!/usr/bin/env python3
"""Renderiza um teaser vertical dinâmico de SIX NAMES sem usar vídeo generativo.

O script transforma um conjunto de imagens líricas em planos com movimento
procedural, cortes alinhados à grade de BPM, variação de luz e vignette. O
áudio é muxado no final por FFmpeg; nenhuma imagem é repetida dentro de um
mesmo plano e hashes proibidos podem ser bloqueados pelo manifesto.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image", type=Path, action="append", dest="images", help="Imagem lírica explícita; pode ser repetido.")
...
def discover_images(image_dir: Path | None, explicit: list[Path] | None, forbidden: set[str]) -> list[Path]:
    if explicit:
    ...
    for path in images:
        ...
        if digest in forbidden:
            raise SystemExit(f"Imagem bloqueada pelo manifesto: {path}")
        if digest in seen:
            raise SystemExit(f"Imagem duplicada no conjunto de entrada: {path}")
        seen.add(digest)
    if len(images) < 4:
        raise SystemExit("SIX NAMES requer pelo menos 4 imagens líricas distintas para evitar repetição de planos.")
    return images
...
def animated_frame(source: Image.Image, progress: float, motion: str, direction: int, size: tuple[int, int]) -> Image.Image:
    width, height = size
    ...
    scale = 1.04 + 0.10 * progress if motion in {"push_in", "zoom"} else 1.08
    ...
    vignette = ImageEnhance.Brightness(vignette).enhance(255)
    # A blurred white ellipse gives a soft, cinematic edge falloff.
...
def render_frames(shots: list[Shot], temp_dir: Path, fps: int, size: tuple[int, int]) -> None:
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ktd_six_names_") as work:
        temp_dir = Path(work)
