...
A direção atual do videoclipe segue exclusivamente o modelo vivo e o formato vertical da pasta oficial [`assets/video/aprovados`](assets/video/aprovados): **KTD cantando em cena com lip-sync visível**, respiração, emoção e gestual coerentes com a letra, além de câmera em movimento, cenários reativos, chuva, água, vapor, fogo, luzes e montagem narrativa em 9:16, 720×1280 e 24 fps. A versão baseada em imagens estáticas e o tratamento landscape anterior são históricos e não são a autoridade criativa.

O preview anterior foi classificado como **mood film sem lip-sync** e não é um videoclipe aprovado. O manifest está deliberadamente em `performance_rebuild_required`; nenhuma cena muda será promovida. O contrato de aceite está em [`docs/ktd-fire-in-the-flood-performance-direction-v1.md`](docs/ktd-fire-in-the-flood-performance-direction-v1.md), o gate visual de identidade está em [`docs/ktd-fire-in-the-flood-identity-fidelity-gate-v1.md`](docs/ktd-fire-in-the-flood-identity-fidelity-gate-v1.md), e a fila regenerada em [`data/releases/fire-in-the-flood-10s-generation-queue-v1.json`](data/releases/fire-in-the-flood-10s-generation-queue-v1.json) separa as 14 cenas vocais das 3 cenas instrumentais. Uma geração só pode avançar se mantiver o mesmo rosto, barba, heterocromia, anatomia e mapa de tatuagens em toda a duração do plano.

A decupagem oficial de 168 segundos está em [`docs/ktd-fire-in-the-flood-10s-scene-script-v4.md`](docs/ktd-fire-in-the-flood-10s-scene-script-v4.md), alinhada à letra v4 de [`docs/ktd-main-single-rework-lyrics.md`](docs/ktd-main-single-rework-lyrics.md) e organizada em 16 cenas de 10 segundos mais um encerramento instrumental de 8 segundos. O manifest de produção v4 está em [`data/releases/fire-in-the-flood-10s-scene-manifest-v1.json`](data/releases/fire-in-the-flood-10s-scene-manifest-v1.json), com status `performance_rebuild_required`, e a fila de prompts performáticos para geração posterior está em [`data/releases/fire-in-the-flood-10s-generation-queue-v1.json`](data/releases/fire-in-the-flood-10s-generation-queue-v1.json). A ficha técnica canônica está em [`data/releases/fire-in-the-flood-official-approved-format-v1.json`](data/releases/fire-in-the-flood-official-approved-format-v1.json), e a rebaseline da pasta oficial, em [`docs/ktd-fire-in-the-flood-approved-folder-rebaseline-v1.md`](docs/ktd-fire-in-the-flood-approved-folder-rebaseline-v1.md). O pipeline de montagem/mux com a master v4 é [`scripts/assemble_fire_in_the_flood_10s.py`](scripts/assemble_fire_in_the_flood_10s.py); o estado de geração desacoplada está documentado em [`docs/ktd-fire-in-the-flood-background-generation.md`](docs/ktd-fire-in-the-flood-background-generation.md).
